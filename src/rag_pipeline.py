...
class GeminiLLMClient:
    ...
    def generate(self, prompt: str) -> str:
        ...
        if not self._is_available:
            return (
                f"[GEMINI_API_KEY Missing / Not Configured]\n\n"
                f"Based on the retrieved enterprise documents, here is the context summary:\n\n"
                f"{prompt.split('--- RETRIEVED ENTERPRISE CONTEXT ---')[-1].split('--- QUESTION ---')[0].strip()[:600]}...\n\n"
                f"*(Note: To generate full conversational answers with Google Gemini, set your `GEMINI_API_KEY` in the sidebar or `.env` file.)*"
            )
...
class RAGPipeline:
    ...
    def __init__(
        ...
        self.retriever = FAISSRetriever(self.embedding_manager, index_dir=index_dir)
    ...
    def ingest_documents(
        ...
        num_indexed = self.retriever.build_index(chunks, force_rebuild=force_rebuild)
        return {
            "status": "success",
    ...
    def _build_contextual_query(self, user_query: str) -> str:
        ...
        follow_up_tokens = ["what about", "and for", "how about", "what if", "it", "they", "them", "that", "this"]
        lower_q = user_query.lower()

        needs_context = any(tok in lower_q for tok in follow_up_tokens) or len(user_query.split()) <= 4
    ...
    def _build_prompt(self, query: str, chunks: List[SearchResult]) -> str:
        ...
        for idx, chunk in enumerate(chunks, start=1):
            ...
            context_blocks.append(
                f"[Source {idx}]: File: {fname} | Page: {page} | Section: {sec}\n{chunk.text}"
            )
        formatted_context = "\n\n".join(context_blocks)
        ...
        if len(self.conversation_history) > 0:
            ...
            formatted_turns = [f"{t.role.capitalize()}: {t.content}" for t in turns]
            history_context = "Conversation History (for context only):\n" + "\n".join(formatted_turns) + "\n\n"

        prompt = f"""You are the official Enterprise AI Document Assistant.
...
{history_context}--- RETRIEVED ENTERPRISE CONTEXT ---
{formatted_context}
...
--- GROUNDED ANSWER ---"""
    ...
    def query(
        ...
        retrieved_chunks = self.retriever.retrieve(
            ...
            top_k=top_k,
            similarity_threshold=None,  # We retrieve top_k and evaluate threshold
        )
        ...
        # If top score is below threshold or no chunks returned, avoid LLM guessing
        if not retrieved_chunks or top_score < thresh:
            answer = (
                f"{INSUFFICIENT_INFO_RESPONSE}\n\n"
                f"The semantic similarity score ({top_score:.2f}) was below the confidence threshold ({thresh:.2f}). "
                f"No relevant enterprise document could be matched to: '{user_query}'."
            )
            resp = RAGResponse(
                answer=answer,
                sources=[],
            ...
            self.conversation_history.append(ConversationTurn(role="user", content=user_query))
            self.conversation_history.append(ConversationTurn(role="assistant", content=answer, sources=[]))
            return resp
        ...
        self.conversation_history.append(
            ConversationTurn(role="assistant", content=llm_answer, sources=sources)
        )
