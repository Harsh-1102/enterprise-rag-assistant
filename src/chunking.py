...
Splits extracted documents into optimal overlapping chunks for dense vector
embedding. Preserves parent document metadata:
...
class DocumentChunker:
    """
    Configurable recursive character splitter with overlap and
    ...
    def _split_text_recursive(
        ...
        for piece in splits:
            if not piece.strip():
                continue
            candidate = (
                f"{current_piece}{sep}{piece}" if current_piece else piece
            )
            if len(candidate) <= self.chunk_size:
                ...
                if len(piece) > self.chunk_size:
                    ...
                    sub_pieces = self._split_text_recursive(
                        piece, separators[1:]
                    )
        ...
        while i < len(good_splits):
            ...
            j = i + 1
            while j < len(good_splits):
                added_len = len(combined) + len(sep) + len(good_splits[j])
                if added_len <= self.chunk_size:
    ...
    def chunk_document(
        self, document: Document, start_index: int = 0
    ) -> List[Chunk]:
    ...
    def chunk_documents(self, documents: List[Document]) -> List[Chunk]:
        """Batch chunks Document objects preserving global chunk index."""
